...
public class TintColorExtractor {
    ...
    public static Bucket processImage(NativeImage image) {
        int width = image.getWidth();
        int height = image.getHeight();
        int[] pixels=image.getPixels();
    	return processImage(width,height,pixels);
    }
    /**
     ...
     */
    public static Bucket processImage(int width,int height,int[] pixels) {
    ...
    public static int extractTintColor(Bucket result, int quantColors) {
        ...
        for (Bucket bucket : buckets) {
            if (bucket.totalWeight == 0) continue;
    ...
    private static Set<Bucket> medianCut(Bucket initalbucket, int targetBuckets) {
        ...
        while (buckets.size() < targetBuckets) {
            ...
            for (Bucket bucket : buckets) {
                if (bucket.colors.size() < 2) continue;
                ...
                for (WeightedColor c : bucket.colors) {
                	Color clr=c.color;
                    if (clr.r < minR) minR = clr.r;
                    if (clr.r > maxR) maxR = clr.r;
                    if (clr.g < minG) minG = clr.g;
                    if (clr.g > maxG) maxG = clr.g;
                    if (clr.b < minB) minB = clr.b;
                    if (clr.b > maxB) maxB = clr.b;
                }
    ...
    public static void main(String[] args) throws IOException {
		...
		Bucket procx=processImage(fc.width,fc.height,fc.pixels);
		extractTintColor(procx,12);
    	long start=System.nanoTime();
    	for(int o=0;o<100;o++) {
	    	for(ImageResult f:ir) {
	    		
	    		Bucket proc=processImage(f.width,f.height,f.pixels);
	    		f.color=0;
